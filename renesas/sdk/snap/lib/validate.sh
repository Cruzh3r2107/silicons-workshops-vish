...
snap_validate() {
    ...
    if command -v unsquashfs >/dev/null 2>&1; then
        ...
        # 3. expected kernel content present
        local listing
        listing="$(unsquashfs -l "${s}" 2>/dev/null || true)"
        grep -Eq '(kernel\.img|Image|vmlinuz|initrd\.img)' <<<"${listing}" \
            || errors+=("no kernel image/initrd content found inside $(basename "${s}")")
        grep -Eq 'modules' <<<"${listing}" \
            || workshop_warn "no 'modules' entry visible in snap listing (may be expected for some recipes)"
    else
        workshop_warn "unsquashfs not available — skipping snap content validation"
    fi
